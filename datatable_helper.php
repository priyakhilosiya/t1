...
class SSP {
	 ...
	 */
	static function simple ( $request, $table, $primaryKey, $columns,$join = array(),$custom_where = array(),$type="common_model",$custom_fields = array())
	{
		$bindings = array();
		$model = SSP::get_model($type);
	 ...
	 */
	static function get_model ($type)
	{
		$ci = & get_instance();
		$ci->load->model($type, 'model');
		return $ci->model;
	}
